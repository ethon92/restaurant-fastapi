...
def search_restaurants(
    ...
    tags: List[str] = Query([]),    
    city: str = Query("全部"),         
    price_level: str = Query("全部"),  
):
    return service.search(q=q, tags=tags, city=city, price_level=price_level)
...
async def get_restaurants_by_bounds(
    ...
    results = service.get_restaurants_by_bounds(min_lat, max_lat, min_lng, max_lng)
